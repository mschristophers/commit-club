...
async function main() {
  ...
  await viem.waitForTransactionReceipt({ hash: createTx2 });

  // Get commitment ID
  const logs2 = await viem.getLogs({
  ...
  const commitmentId2 = logs2[0].args.id;
  console.log("🎯 Commitment ID:", commitmentId2);
  ...
  for (let i = 0; i < 2; i++) {
    ...
    console.log(`✅ Join ${i + 1} completed! TX:`, joinTx);
    await viem.waitForTransactionReceipt({ hash: joinTx });
  }
  ...
  await viem.waitForTransactionReceipt({ hash: checkInTx2 });
  ...
  await viem.waitForTransactionReceipt({ hash: createTx3 });

  // Get commitment ID
  const logs3 = await viem.getLogs({
    address: CONTRACT_ADDRESS,
    event: {
      type: 'event',
      name: 'CommitmentCreated',
  ...
  const commitmentId3 = logs3[0].args.id;
  console.log("🎯 Commitment ID:", commitmentId3);
  ...
  await viem.waitForTransactionReceipt({ hash: joinTx3 });
  ...
  await viem.waitForTransactionReceipt({ hash: checkInTx3 });

  // Wait for deadlines to pass
  console.log("\n⏰ Waiting for deadlines to pass...");
  await new Promise(resolve => setTimeout(resolve, 10000)); // Wait 10 seconds

  // Settle all commitments
  console.log("\n" + "=".repeat(60));
  console.log("💰 SETTLING ALL COMMITMENTS");
  console.log("=".repeat(60));

  const commitmentsToSettle = [commitmentId1, commitmentId2, commitmentId3];
  
  for (let i = 0; i < commitmentsToSettle.length; i++) {
    const commitmentId = commitmentsToSettle[i];
    console.log(`\n💰 Settling commitment ${commitmentId}...`);
    
    try {
      const settleTx = await commitClub.write.settleCommit([commitmentId]);
      console.log(`✅ Commitment ${commitmentId} settled! TX:`, settleTx);
      await viem.waitForTransactionReceipt({ hash: settleTx });
    } catch (error) {
      console.log(`❌ Failed to settle commitment ${commitmentId}:`, error.message);
    }
  }
  ...
  console.log("✅ 3 Settled events (with different outcomes)");
